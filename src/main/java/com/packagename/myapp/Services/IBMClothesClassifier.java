...
public class IBMClothesClassifier implements IClothesClassifier {
    ...
    private final String[] models = {"ClothModel_1452037569","ColorModel_1451624520","PatternModel_22316457"};
    ...
    private Map<String,String> classify(String imageUrl, int recognizer){
        ...
        ClassifyOptions.Builder classifyOptions = new ClassifyOptions.Builder()
                .url(imageUrl)
                .addClassifierId(models[recognizer*2])
                .threshold(0f);
        if(recognizer*2+1 < models.length){
            classifyOptions.addClassifierId(models[recognizer*2+1]);
        }

        ClassifiedImages results = visualRecognition.classify(classifyOptions.build()).execute().getResult();
